...
describe('ContradictionResolver', () => {
  ...
  it('evaluates importance_equals condition', async () => {
      ...
      [
        {
          ...
          condition: {
            ...
            value: 'critical' as import('@reaatech/agent-memory-core').MemoryImportance,
          },
          action: { type: 'prefer_new' },
        },
      ],
    ...
    const criticalMem = createMemory('new', 0.9, {
      importance: 'critical' as import('@reaatech/agent-memory-core').MemoryImportance,
    });
    const existing = createMemory('old', 0.8);
  ...
  it('evaluates memory_type_in condition', async () => {
      ...
      [
        {
          ...
          condition: {
            type: 'memory_type_in',
            values: ['preference' as import('@reaatech/agent-memory-core').MemoryType],
          },
          action: { type: 'prefer_new' },
        },
      ],
    ...
    const prefMem = createMemory('new', 0.9, {
      type: 'preference' as import('@reaatech/agent-memory-core').MemoryType,
    });
    const existing = createMemory('old', 0.8);
